...
import { MapPin, Calendar, Users, Filter, Star, Car, Wifi, Music, PawPrint, Wind, ChevronDown } from 'lucide-react';
...
function SearchContent() {
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A]">
      ...
      <div className="pt-20">
        {/* Search bar */}
        <div className="bg-[#111111] border-b border-white/[0.05] py-5 px-4 sticky top-16 z-40">
          <div className="max-w-5xl mx-auto flex flex-col sm:flex-row gap-3">
            <div className="flex items-center gap-2 flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-2.5">
              ...
              <input value={filters.pickup} onChange={e => setFilters(f => ({ ...f, pickup: e.target.value }))}
                placeholder="From" className="bg-transparent text-white text-sm outline-none w-full placeholder-white/30" />
            </div>
            <div className="flex items-center gap-2 flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-2.5">
              <MapPin className="w-4 h-4 text-white/30 shrink-0" />
              <input value={filters.drop} onChange={e => setFilters(f => ({ ...f, drop: e.target.value }))}
                placeholder="To" className="bg-transparent text-white text-sm outline-none w-full placeholder-white/30" />
            </div>
            ...
            <div className="flex gap-2">
              <button onClick={() => setShowFilters(!showFilters)}
                className="flex items-center gap-2 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white/60 hover:bg-white/5 transition-all">
                <Filter className="w-4 h-4" /> Filters
              </button>
              <button onClick={handleSearch}
                className="bg-[#00C853] hover:bg-[#00A846] text-black font-semibold text-sm px-6 py-2.5 rounded-xl transition-colors">
                Search
              </button>
            </div>
          </div>
          ...
          {showFilters && (
            <div className="max-w-5xl mx-auto mt-3 flex flex-wrap gap-3 pt-3 border-t border-white/[0.05]">
              ...
              <button
                onClick={() => setFilters(f => ({ ...f, womenOnly: !f.womenOnly }))}
        ...
        <div className="max-w-5xl mx-auto px-4 py-8">
          {isLoading ? (
            ...
            <div className="text-center py-20">
              ...
              <Link href="/rides/offer" className="inline-flex items-center gap-2 mt-6 text-sm text-[#00C853] hover:text-[#00A846]">
...
export default function SearchPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#0A0A0A] flex items-center justify-center"><div className="text-white/40">Loading...</div></div>}>
      <SearchContent />
    </Suspense>
  );
}
